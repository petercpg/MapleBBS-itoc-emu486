...
bw_init()
{
  ...
  move(b_lines - 1, 0);
  /*  對奕模式  */
  /*  (Enter)落子 (TAB)切換棋盤/交談 (^P)讓手 (^C)重玩 (^D)離開          \033[m */
...
ch_init()
{
  ...
  move(b_lines - 1, 0);
  /*  對奕模式  */
  /*  (Enter)翻子/移動 (TAB)切換棋盤/交談 (^S)認輸 (^C)重玩 (^D)離開     \033[m */
...
ch_recv()
{
  ...
  case 'E':
    ...
    mapTurn = myTurn;
    /* \033[1;32m▽對方移動 %s(%d, %c) 吃 %s(%d, %c)\033[m */
  ...
  case 'M':
    ...
    Board[row2][col2] = ch;
    mapTurn = myTurn;
    /* \033[1;37m▽對方移動 %s(%d, %c) 至 (%d, %c)\033[m */
...
ch_Mv0()			/* for 軍棋 */
{
  ...
  if (way == 1)
  {
    ...
    if (!do_send(buf))
      return DISCONNECT;
    /* \033[1;36m▼我方移動 %s(%d, %c) 至 (%d, %c)\033[m */
  ...
  else if (way == 2)
  {
    ...
    if (!do_send(buf))
      return DISCONNECT;
    /* \033[1;32m▼我方移動 %s(%d, %c) 吃 %s(%d, %c)\033[m */
  ...
  int sock, later;
{
  ...
  if (!later)
  {
    ...
    /* 想下哪種棋 (1)黑白棋 (2)五子棋 (3)圍棋 (4)軍棋 (5)暗棋 (Q)取消？[Q]  */
  ...
  else
  {
    ...
    /* ★ 對方要求進入對奕模式選擇中，請稍候 \033[5m...\033[m */
