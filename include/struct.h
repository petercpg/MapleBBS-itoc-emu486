/*-------------------------------------------------------*/
/* include/struct.h     ( NTHU CS MapleBBS Ver 2.36 )    */
/*-------------------------------------------------------*/
...
typedef struct
{
  ...
  time32_t firstlogin;		/* 第一次上站時間 */
  time32_t lastlogin;		/* 上一次上站時間 */
  time32_t tcheck;		/* 上次 check 信箱/朋友名單的時間 */
  time32_t tvalid;		/* 若停權，停權期滿的時間；
                                   若未停權且通過認證，通過認證的時間；
...
{
  time32_t logtime;
  time32_t used_time[M_MAX + 1];	/* itoc.010901: depend on mode.h */
} UMODELOG;
