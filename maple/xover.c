/*-------------------------------------------------------*/
/* maple/xover.c        ( NTHU CS MapleBBS Ver 3.10 )    */
/*-------------------------------------------------------*/
...
#ifdef MY_FAVORITE
/* 快速切換：(A)精華 (B)文章 (C)看板 (F)最愛 (M)信件 (U)使用者 (W)水球： */
#define MSG_ZONE_SWITCH	"\xA7\xD6\xB3\x74\xA4\xC1\xB4\xAB\xA1\x47(A)\xBA\xEB\xB5\xD8 (B)\xA4\xE5\xB3\xB9 (C)\xAC\xDD\xAA\x4F (F)\xB3\xCC\xB7\x52 (M)\xAB\x48\xA5\xF3 (U)\xA8\xCF\xA5\xCE\xAA\xCC (W)\xA4\xF4\xB2\x79\xA1\x47"
#else
/* 快速切換：(A)精華 (B)文章 (C)看板 (M)信件 (U)使用者 (W)水球： */
#define MSG_ZONE_SWITCH	"\xA7\xD6\xB3\x74\xA4\xC1\xB4\xAB\xA1\x47(A)\xBA\xEB\xB5\xD8 (B)\xA4\xE5\xB3\xB9 (C)\xAC\xDD\xAA\x4F (M)\xAB\x48\xA5\xF3 (U)\xA8\xCF\xA5\xCE\xAA\xCC (W)\xA4\xF4\xB2\x79\xA1\x47"
#endif
  ...
  void (*fdel) ();			/* 除了刪除記錄以外，還要做些什麼事 */
{
  ...
  /* [設定刪除範圍] 起點： */
  vget(b_lines, 0, "[\xB3\x5D\xA9\x77\xA7\x52\xB0\xA3\xBD\x64\xB3\xF2] \xB0\x5F\xC2\x49\xA1\x47", ans, 6, DOECHO);
  head = atoi(ans);
  if (head <= 0)
  {
    /* 起點有誤 */
    zmsg("\xB0\x5F\xC2\x49\xA6\xB3\xBB\x7E");
    return XO_FOOT;
  }

  /* 終點： */
  vget(b_lines, 28, "\xB2\xD7\xC2\x49\xA1\x47", ans, 6, DOECHO);
  tail = atoi(ans);
  if (tail < head)
  {
    /* 終點有誤 */
    zmsg("\xB2\xD7\xC2\x49\xA6\xB3\xBB\x7E");
    return XO_FOOT;
  }
  ...
  void (*fdel) ();			/* 除了刪除記錄以外，還要做些什麼事 */
{
  ...
  /* 確定要刪除 %d 篇標籤嗎(Y/N)？[N]  */
  sprintf(buf, "\xBD\x54\xA9\x77\xAD\x6E\xA7\x52\xB0\xA3 %d \xBD\x67\xBC\xD0\xC5\xD2\xB6\xDC(Y/N)\xA1\x48[N] ", TagNum);
  if (vans(buf) != 'y')
    return XO_FOOT;
  ...
  XO *xo;
{
  ...
  /* 拷貝到暫存檔 */
  tag = AskTag("\xAB\xFE\xA8\xA9\xA8\xEC\xBC\xC8\xA6\x73\xC0\xC9");
  if (tag < 0)
    return XO_FOOT;
  ...
  fclose(fp);
  /* 拷貝完成 */
  zmsg("\xAB\xFE\xA8\xA9\xA7\xB9\xA6\xA8");
  ...
  XO *xo;
{
  ...
  /* 目的地： */
  if (!vget(b_lines, 0, "\xA5\xD8\xAA\xBA\xA6\x61\xA1\x47", rcpt, sizeof(rcpt), GCARRY))
    return XO_FOOT;
  ...
  else
  {
    ...
    {
      /* 您無法寄信到站外 */
      vmsg("\xB1\x7A\xB5\x4C\xAA\x6B\xB1\x48\xAB\x48\xA8\xEC\xAF\xB8\xA5\x7E");
      return XO_FOOT;
    }
  ...
  do
  {
	...
	{
	  /* [精 選 集] */
	  strcpy(mhdr.owner, "[\xBA\xEB \xBF\xEF \xB6\xB0]");
  ...
  /* 部份信件無法寄達 */
  zmsg(cc >= 0 ? msg_sent_ok : "\xB3\xA1\xA5\xF7\xAB\x48\xA5\xF3\xB5\x4C\xAA\x6B\xB1\x48\xB9\x46");
  ...
  int op;
{
  ...
  else if (op & RS_UNREAD)
  {
    ...
    /* 向前找尋 0)首篇未讀 1)末篇已讀  */
    if (!vget(b_lines, 0, "\xA6\x56\xAB\x65\xA7\xE4\xB4\x4D 0)\xAD\xBA\xBD\x67\xA5\xBC\xC5\xAA 1)\xA5\xBD\xBD\x67\xA4\x77\xC5\xAA ", s_unread, sizeof(s_unread), GCARRY))
      return XO_FOOT;
  ...
  else if (!(op & (RS_THREAD | RS_SEQUENT | RS_MARKED)))
  {
    ...
    /* ↓ */
    /* ↑ */
    sprintf(query = buf, "\xB7\x6A\xB4\x4D%s(%s)\xA1\x47", title, (step > 0) ? "\xA1\xF5" : "\xA1\xF4");
  ...
  int zone;			/* itoc.010403: 把 zone 也傳進來 */
{
  ...
  buf[1] = '\0';
  /* 跳至第幾項： */
  vget(b_lines, 0, "\xB8\xF5\xA6\xDC\xB2\xC4\xB4\x58\xB6\xB5\xA1\x47", buf, sizeof(buf), GCARRY);
