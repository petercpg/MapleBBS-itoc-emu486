/*-------------------------------------------------------*/
/* maple/post.c         ( NTHU CS MapleBBS Ver 2.39 )    */
/*-------------------------------------------------------*/
  ...
  int bno;			/* 要轉去的看板 */
{
  ...
  if (checksum_find(fpath))
  {
    ...
    board_main();
    /* Cross-Post 停權 */
    mail_self(FN_ETC_CROSSPOST, str_sysop, "Cross-Post \xB0\xB1\xC5\x76", 0);
    /* 您因為過度 Cross-Post 已被停權 */
    vmsg("\xB1\x7A\xA6\x5D\xAC\xB0\xB9\x4C\xAB\xD7 Cross-Post \xA4\x77\xB3\x51\xB0\xB1\xC5\x76");
    return 1;
  }
  return 0;
}
  ...
  char *title;
{
  ...
  if (!(bbstate & STAT_POST))
  {
    ...
    if (cuser.lastlogin - cuser.firstlogin < 3 * 86400)
      /* 新手上路，三日後始可張貼文章 */
      vmsg("\xB7\x73\xA4\xE2\xA4\x57\xB8\xF4\xA1\x41\xA4\x54\xA4\xE9\xAB\xE1\xA9\x6C\xA5\x69\xB1\x69\xB6\x4B\xA4\xE5\xB3\xB9");
    else
#endif      
      /* 對不起，您沒有在此發表文章的權限 */
      vmsg("\xB9\xEF\xA4\xA3\xB0\x5F\xA1\x41\xB1\x7A\xA8\x53\xA6\xB3\xA6\x62\xA6\xB9\xB5\x6F\xAA\xED\xA4\xE5\xB3\xB9\xAA\xBA\xC5\x76\xAD\xAD");
    return XO_FOOT;
  }
  ...
  /* 發表文章於【 %s 】看板 */
  prints("\xB5\x6F\xAA\xED\xA4\xE5\xB3\xB9\xA9\xF3\xA1\x69 %s \xA1\x6A\xAC\xDD\xAA\x4F", currboard);
...
#define NUM_PREFIX 6
    ...
    /* [問題]  */
    /* [測試]  */
    char *prefix[NUM_PREFIX] = {"[\xA4\xBD\xA7\x69] ", "[\xB7\x73\xBB\x44] ", "[\xB6\xA2\xB2\xE1] ", "[\xA4\xE5\xA5\xF3] ", "[\xB0\xDD\xC3\x44] ", "[\xB4\xFA\xB8\xD5] "};

    move(21, 0);
    /* 類別： */
    outs("\xC3\xFE\xA7\x4F\xA1\x47");
    for (mode = 0; mode < NUM_PREFIX; mode++)
      prints("%d.%s", mode + 1, prefix[mode]);

    /* 請選擇文章類別（按 Enter 跳過）： */
    mode = vget(20, 0, "\xBD\xD0\xBF\xEF\xBE\xDC\xA4\xE5\xB3\xB9\xC3\xFE\xA7\x4F\xA1\x5D\xAB\xF6 Enter \xB8\xF5\xB9\x4C\xA1\x5E\xA1\x47", fpath, 3, DOECHO) - '1';
  ...
  clear();
  /* 順利貼出文章， */
  outs("\xB6\xB6\xA7\x51\xB6\x4B\xA5\x58\xA4\xE5\xB3\xB9\xA1\x41");

  if (currbattr & BRD_NOCOUNT || wordsnum < 30)
  {				/* itoc.010408: 以此減少灌水現象 */
    /* 文章不列入紀錄，敬請包涵。 */
    outs("\xA4\xE5\xB3\xB9\xA4\xA3\xA6\x43\xA4\x4A\xAC\xF6\xBF\xFD\xA1\x41\xB7\x71\xBD\xD0\xA5\x5D\xB2\x5B\xA1\x43");
  }
  else
  {
    ...
    /* 這是您的第 %d 篇文章，得 %d 銀。 */
    prints("\xB3\x6F\xAC\x4F\xB1\x7A\xAA\xBA\xB2\xC4 %d \xBD\x67\xA4\xE5\xB3\xB9\xA1\x41\xB1\x6F %d \xBB\xC8\xA1\x43", ++cuser.numposts, mode);
    addmoney(mode);
  }
  ...
  if (curredit & EDIT_BOTH)
  {
    ...
    /* \n\n成功回應至作者信箱 */
    /* \n\n作者無法收信 */
    outs(mode >= 0 ? "\n\n\xA6\xA8\xA5\x5C\xA6\x5E\xC0\xB3\xA6\xDC\xA7\x40\xAA\xCC\xAB\x48\xBD\x63" : "\n\n\xA7\x40\xAA\xCC\xB5\x4C\xAA\x6B\xA6\xAC\xAB\x48");
  }
  ...
  HDR *hdr;
{
  ...
  switch (vans("\xA1\xB6 \xA6\x5E\xC0\xB3\xA6\xDC (F)\xAC\xDD\xAA\x4F (M)\xA7\x40\xAA\xCC\xAB\x48\xBD\x63 (B)\xA4\x47\xAA\xCC\xAC\xD2\xAC\x4F (Q)\xA8\xFA\xAE\xF8\xA1\x48[F] "))
  {
  case 'm':
  ...
  int cc;			/* 印出最多 cc - 1 字的標題 */
{
  ...
  /* ◇ */
  /* \033[1;32m◆ */
  static char *type[6] = {"Re", "Fw", "\xA1\xBA", "\033[1;33m=>", "\033[1;33m=>", "\033[1;32m\xA1\xBB"};
  uschar *title, *mark;
  int ch, len;
  int in_chi;		/* 1: 在中文字中 */
#ifdef HAVE_DECLARE
  ...
  XO *xo;
{
  ...
  if (max <= 0)
  {
    ...
    {
      /* 要新增資料嗎(Y/N)？[N]  */
      if (vans("\xAD\x6E\xB7\x73\xBC\x57\xB8\xEA\xAE\xC6\xB6\xDC(Y/N)\xA1\x48[N] ") == 'y')
	return post_add(xo);
    }
    else
    {
      /* 本看板尚無文章 */
      vmsg("\xA5\xBB\xAC\xDD\xAA\x4F\xA9\x7C\xB5\x4C\xA4\xE5\xB3\xB9");
    }
    return XO_QUIT;
  }
  ...
  XO *xo;
{
  ...
  /* ╳ */
  /* ○ */
  prints(NECKER_POST, d_cols, "", currbattr & BRD_NOSCORE ? "\xA2\xAE" : "\xA1\xB3", bshm->mantime[currbno]);
  return post_body(xo);
}
  ...
  XO *xo;
{
  ...
  ans = vans("\xB3\x5D\xA9\x77\xA9\xD2\xA6\xB3\xA4\xE5\xB3\xB9 (U)\xA5\xBC\xC5\xAA (V)\xA4\x77\xC5\xAA (W)\xAB\x65\xA4\x77\xC5\xAA\xAB\xE1\xA5\xBC\xC5\xAA (Q)\xA8\xFA\xAE\xF8\xA1\x48[Q] ");
  ...
  XO *xo;
{
  ...
  /* 精華區 */
  XoGem(fpath, "\xBA\xEB\xB5\xD8\xB0\xCF", level);
  return post_init(xo);
}
  ...
  XO *xo;
{
  ...
  {
    /* 本看板尚無「進板畫面」 */
    vmsg("\xA5\xBB\xAC\xDD\xAA\x4F\xA9\x7C\xB5\x4C\xA1\x75\xB6\x69\xAA\x4F\xB5\x65\xAD\xB1\xA1\x76");
    return XO_FOOT;
  }
  ...
  XO *xo;
{
  ...
  if (!ask_board(xboard, BRD_W_BIT, "\n\n\033[1;33m\xBD\xD0\xAC\x44\xBF\xEF\xBE\x41\xB7\xED\xAA\xBA\xAC\xDD\xAA\x4F\xA1\x41\xA4\xC1\xA4\xC5\xC2\xE0\xBF\xFD\xB6\x57\xB9\x4C\xA4\x54\xAA\x4F\xA1\x43\033[m\n\n") ||
  ...
  method = (HAS_PERM(PERM_ALLBOARD) || (!tag && is_author(hdr))) &&
    /* (1)原文轉載 (2)轉錄文章？[1]  */
    (vget(2, 0, "(1)\xAD\xEC\xA4\xE5\xC2\xE0\xB8\xFC (2)\xC2\xE0\xBF\xFD\xA4\xE5\xB3\xB9\xA1\x48[1] ", buf, 3, DOECHO) != '2') ? 0 : 1;
  ...
  if (!tag)	/* lkchu.981201: 整批轉錄就不要一一詢問 */
  {
    ...
    /* 標題： */
    if (!vget(2, 0, "\xBC\xD0\xC3\x44\xA1\x47", ve_title, TTLEN + 1, GCARRY))
      return XO_HEAD;
  }

#ifdef HAVE_REFUSEMARK    
  /* (S)存檔 (L)站內 (X)密封 (Q)取消？[Q]  */
  rc = vget(2, 0, "(S)\xA6\x73\xC0\xC9 (L)\xAF\xB8\xA4\xBA (X)\xB1\x4B\xAB\xCA (Q)\xA8\xFA\xAE\xF8\xA1\x48[Q] ", buf, 3, LCECHO);
  if (rc != 'l' && rc != 's' && rc != 'x')
#else
  /* (S)存檔 (L)站內 (Q)取消？[Q]  */
  rc = vget(2, 0, "(S)\xA6\x73\xC0\xC9 (L)\xAF\xB8\xA4\xBA (Q)\xA8\xFA\xAE\xF8\xA1\x48[Q] ", buf, 3, LCECHO);
  if (rc != 'l' && rc != 's')
#endif
    return XO_HEAD;
  ...
  do	/* lkchu.981201: 整批轉錄 */
  {
    ...
    if (method)		/* 一般轉錄 */
    {
	 ...
	 然而 currboard 未必是該精華區的看板。不過不是很重要的問題，所以就不管了 :p */
      /* ※ 本文轉錄自 [%s] %s\n\n */
      fprintf(fpw, "\xA1\xB0 \xA5\xBB\xA4\xE5\xC2\xE0\xBF\xFD\xA6\xDB [%s] %s\n\n", 
	/* 秘密 */
	*dir == 'u' ? cuser.userid : method == 2 ? "\xAF\xB5\xB1\x4B" : tmpboard, 
	/* 信箱 */
	/* 看板 */
	*dir == 'u' ? "\xAB\x48\xBD\x63" : "\xAC\xDD\xAA\x4F");
  ...
  /* 轉錄完成 */
  vmsg("\xC2\xE0\xBF\xFD\xA7\xB9\xA6\xA8");
  return XO_HEAD;
}
  ...
  XO *xo;
{
  ...
  /* 轉達信件給： */
  if (acct_get("\xC2\xE0\xB9\x46\xAB\x48\xA5\xF3\xB5\xB9\xA1\x47", &muser) > 0)
  {
    strcpy(quote_user, hdr->owner);
    strcpy(quote_nick, hdr->nick);
    hdr_fpath(quote_file, xo->dir, hdr);
  ...
  XO *xo;
{
  ...
  /* 確定要刪除待砍文章嗎(Y/N)？[N]  */
  if (vans("\xBD\x54\xA9\x77\xAD\x6E\xA7\x52\xB0\xA3\xAB\xDD\xAC\xE5\xA4\xE5\xB3\xB9\xB6\xDC(Y/N)\xA1\x48[N] ") != 'y')
    return XO_FOOT;
  ...
  XO *xo;
{
  ...
  if (vans(msg_del_ny) == 'y')
  {
	...
	if (cuser.numposts > 0)
	  cuser.numposts--;
	/* %s，您的文章減為 %d 篇 */
	sprintf(buf, "%s\xA1\x41\xB1\x7A\xAA\xBA\xA4\xE5\xB3\xB9\xB4\xEE\xAC\xB0 %d \xBD\x67", MSG_DEL_OK, cuser.numposts);
	vmsg(buf);
      }
      ...
      {
	/* 原列表經刪除後混亂，請重進串接模式！ */
	vmsg("\xAD\xEC\xA6\x43\xAA\xED\xB8\x67\xA7\x52\xB0\xA3\xAB\xE1\xB2\x56\xB6\xC3\xA1\x41\xBD\xD0\xAD\xAB\xB6\x69\xA6\xEA\xB1\xB5\xBC\xD2\xA6\xA1\xA1\x49");
	return XO_QUIT;
      }
      return XO_LOAD;
    }
  ...
  XO *xo;
{
  ...
  {
    /* 原列表經批次刪除後混亂，請重進串接模式！ */
    vmsg("\xAD\xEC\xA6\x43\xAA\xED\xB8\x67\xA7\xE5\xA6\xB8\xA7\x52\xB0\xA3\xAB\xE1\xB2\x56\xB6\xC3\xA1\x41\xBD\xD0\xAD\xAB\xB6\x69\xA6\xEA\xB1\xB5\xBC\xD2\xA6\xA1\xA1\x49");
    return XO_QUIT;
  }
  ...
  XO *xo;
{
  ...
  /* 看板文章拷貝 */
  tag = AskTag("\xAC\xDD\xAA\x4F\xA4\xE5\xB3\xB9\xAB\xFE\xA8\xA9");
  ...
  if (bbstate & STAT_BOARD)
  {
    ...
    if (xo->key == XZ_XPOST)
    {
      /* 檔案標記完成。[注意] 您必須先離開串接模式才能進入精華區。 */
      zmsg("\xC0\xC9\xAE\xD7\xBC\xD0\xB0\x4F\xA7\xB9\xA6\xA8\xA1\x43[\xAA\x60\xB7\x4E] \xB1\x7A\xA5\xB2\xB6\xB7\xA5\xFD\xC2\xF7\xB6\x7D\xA6\xEA\xB1\xB5\xBC\xD2\xA6\xA1\xA4\x7E\xAF\xE0\xB6\x69\xA4\x4A\xBA\xEB\xB5\xD8\xB0\xCF\xA1\x43");
      return XO_FOOT;
    }
    else
#endif
  ...
  /* 檔案標記完成。[注意] 您只能在擔任(小)板主所在或個人精華區貼上。 */
  zmsg("\xC0\xC9\xAE\xD7\xBC\xD0\xB0\x4F\xA7\xB9\xA6\xA8\xA1\x43[\xAA\x60\xB7\x4E] \xB1\x7A\xA5\x75\xAF\xE0\xA6\x62\xBE\xE1\xA5\xF4(\xA4\x70)\xAA\x4F\xA5\x44\xA9\xD2\xA6\x62\xA9\xCE\xAD\xD3\xA4\x48\xBA\xEB\xB5\xD8\xB0\xCF\xB6\x4B\xA4\x57\xA1\x43");
  return XO_FOOT;
}
  ...
  HDR *hdr;
{
  ...
  /* 標 */
  if (!str_ncmp(buf, "\xBC\xD0", 2))		/* 如果有 header 才改 */
  {
    /* 標  題:  */
    /* 標題:  */
    strcpy(buf, buf[2] == ' ' ? "\xBC\xD0  \xC3\x44: " : "\xBC\xD0\xC3\x44: ");
  ...
  XO *xo;
{
  ...
  {
    /* 作者： */
    vget(b_lines, 0, "\xA7\x40\xAA\xCC\xA1\x47", mhdr.owner, 73 /* sizeof(mhdr.owner) */, GCARRY);
		/* Thor.980727: sizeof(mhdr.owner) = 80 會超過一行 */
    /* 暱稱： */
    vget(b_lines, 0, "\xBC\xCA\xBA\xD9\xA1\x47", mhdr.nick, sizeof(mhdr.nick), GCARRY);
    /* 日期： */
  ...
  XO *xo;
{
  ...
  /* ◎ 評分 1)推文 2)唾棄 3)自定推 4)自定呸？[Q]  */
  switch (ans = vans("\xA1\xB7 \xB5\xFB\xA4\xC0 1)\xB1\xC0\xA4\xE5 2)\xB3\xE8\xB1\xF3 3)\xA6\xDB\xA9\x77\xB1\xC0 4)\xA6\xDB\xA9\x77\xA9\x41\xA1\x48[Q] "))
  {
  case '1':
    /* 1m推 */
    verb = "1m\xB1\xC0";
  ...
  case '4':
    /* 請輸入動詞： */
    if (!vget(b_lines, 0, "\xBD\xD0\xBF\xE9\xA4\x4A\xB0\xCA\xB5\xFC\xA1\x47", fpath, 5, DOECHO))
      return XO_FOOT;
    vtlen = strlen(fpath);
  ...
  /* 請輸入理由： */
  if (!vget(b_lines, 0, "\xBD\xD0\xBF\xE9\xA4\x4A\xB2\x7A\xA5\xD1\xA1\x47", reason, maxlen, DOECHO))
    return XO_FOOT;
  ...
  if (currbattr & BRD_ANONYMOUS)
  {
    ...
    if (!vget(b_lines, 0, "\xBD\xD0\xBF\xE9\xA4\x4A\xB1\x7A\xB7\x51\xA5\xCE\xAA\xBAID\xA1\x41\xA4\x5D\xA5\x69\xAA\xBD\xB1\xB5\xAB\xF6[Enter]\xA1\x41\xA9\xCE\xAC\x4F\xAB\xF6[r]\xA5\xCE\xAF\x75\xA6\x57\xA1\x47", userid, IDLEN, DOECHO))
  ...
  if (fp = fopen(fpath, "a"))
  {
    ...
    /* → \033[36m%s \033[3%s\033[m：%-*s%02d/%02d/%02d\n */
    fprintf(fp, "\xA1\xF7 \033[36m%s \033[3%s\033[m\xA1\x47%-*s%02d/%02d/%02d\n", 
