  ...
  int header_len;
{
  ...
  /* 標題 */
  /* 時間 */
  static char header1[LINE_HEADER][LEN_AUTHOR1] = {"\xA7\x40\xAA\xCC",   "\xBC\xD0\xC3\x44",   "\xAE\xC9\xB6\xA1"};
  /* 發信人 */
  /* 標  題 */
  /* 發信站 */
  static char header2[LINE_HEADER][LEN_AUTHOR2] = {"\xB5\x6F\xAB\x48\xA4\x48", "\xBC\xD0  \xC3\x44", "\xB5\x6F\xAB\x48\xAF\xB8"};
  int i;
  char *ptr, *word;
...
more_slideshow()
{
  ...
  if (!slideshow)
  {
    ...
    if (ch == '@')
    {
      /* 請選擇放映的速度 1(最慢)～9(最快)？播放中按任意鍵可停止播放： */
      slideshow = vans("\xBD\xD0\xBF\xEF\xBE\xDC\xA9\xF1\xAC\x4D\xAA\xBA\xB3\x74\xAB\xD7 1(\xB3\xCC\xBA\x43)\xA1\xE3""9(\xB3\xCC\xA7\xD6)\xA1\x48\xBC\xBD\xA9\xF1\xA4\xA4\xAB\xF6\xA5\xF4\xB7\x4E\xC1\xE4\xA5\x69\xB0\xB1\xA4\xEE\xBC\xBD\xA9\xF1\xA1\x47") - '0';
...
re_key:
	...
	shift = HUNT_MASK | HUNT_NEXT;
      ...
      /* 搜尋： */
      else if (vget(b_lines, 0, "\xB7\x6A\xB4\x4D\xA1\x47", hunt, sizeof(hunt), DOECHO))
      {
	str_lowest(hunt, hunt);
	shift = HUNT_MASK | HUNT_START;
      }
