/*-------------------------------------------------------*/
/* maple/favor.c        ( NTHU CS MapleBBS Ver 3.10 )    */
/*-------------------------------------------------------*/
/* target : 我的最愛					 */
/* create : 00/06/16					 */
/* update :   /  /  					 */
  ...
  MF *mf;
{
  ...
  if (mftype & MF_FOLDER)
  {
    ...
    /* ◆ */
    prints("%6d%c  %s %s\n", num, mftype & MF_MARK ? ')' : ' ', "\xA1\xBB", mf->title);
  }
  else if (mftype & MF_BOARD)
  {
    if ((bno = brd_bno(mf->xname)) >= 0)
      ...
      /*          \033[36m<%s 已改名或被刪除，請將本捷徑刪除>\033[m\n */
      prints("         \033[36m<%s \xA4\x77\xA7\xEF\xA6\x57\xA9\xCE\xB3\x51\xA7\x52\xB0\xA3\xA1\x41\xBD\xD0\xB1\x4E\xA5\xBB\xB1\xB6\xAE\x7C\xA7\x52\xB0\xA3>\033[m\n", mf->xname);
  }
  ...
  XO *xo;
{
  ...
  if (max <= 0)
  {
    /* 我的最愛 (A)新增 (P)貼上 (Q)離開 [Q]  */
    max = vans("\xA7\xDA\xAA\xBA\xB3\xCC\xB7\x52 (A)\xB7\x73\xBC\x57 (P)\xB6\x4B\xA4\x57 (Q)\xC2\xF7\xB6\x7D [Q] ");
    switch (max)
    {
    case 'a':
  ...
  XO *xo;
{
  ...
  /* 新增 (B)看板捷徑 (F)卷宗 (G)精華區捷徑 (L)分隔線 [Q]  */
  ans = vans("\xB7\x73\xBC\x57 (B)\xAC\xDD\xAA\x4F\xB1\xB6\xAE\x7C (F)\xA8\xF7\xA9\x76 (G)\xBA\xEB\xB5\xD8\xB0\xCF\xB1\xB6\xAE\x7C (L)\xA4\xC0\xB9\x6A\xBD\x75 [Q] ");
  ...
  if (ans == 'b' || ans == 'g')
  {
    ...
    if (ans == 'g')
      /* %s 板 精華區捷徑 */
      sprintf(mf.title, "%s \xAA\x4F \xBA\xEB\xB5\xD8\xB0\xCF\xB1\xB6\xAE\x7C", mf.xname);
  }
  else /* if (ans == 'f' || ans == 'l') */
  {
    /* 標題： */
    if (!vget(b_lines, 0, "\xBC\xD0\xC3\x44\xA1\x47", mf.title, BTLEN + 1, DOECHO))
      return XO_FOOT;
  ...
  XO *xo;
{
  ...
  /* 標題： */
  vget(b_lines, 0, "\xBC\xD0\xC3\x44\xA1\x47", xmf.title, BTLEN + 1, GCARRY);
  ...
  XO *xo;
{
  ...
  if (type & MF_BOARD)		/* 看板捷徑 */
  {
    ...
    if ((bno = brd_bno(xname)) < 0 || !(brd_bits[bno] & BRD_R_BIT))
    {
      ...
      /* 本看板已被刪除或您沒有權限閱讀本看板，系統將自動移除捷徑 */
      vmsg("\xA5\xBB\xAC\xDD\xAA\x4F\xA4\x77\xB3\x51\xA7\x52\xB0\xA3\xA9\xCE\xB1\x7A\xA8\x53\xA6\xB3\xC5\x76\xAD\xAD\xBE\x5C\xC5\xAA\xA5\xBB\xAC\xDD\xAA\x4F\xA1\x41\xA8\x74\xB2\xCE\xB1\x4E\xA6\xDB\xB0\xCA\xB2\xBE\xB0\xA3\xB1\xB6\xAE\x7C");
      return mf_load(xo);
    }
  ...
  else if (type & MF_GEM)	/* 精華區捷徑 */
  {
    ...
    /* 精華區 */
    XoGem(fpath, "\xBA\xEB\xB5\xD8\xB0\xCF", type);
    return mf_init(xo);
  }
  ...
  else if (type & MF_CLASS)	/* 分類群組 */
  {
    if (!MFclass_browse(xname))
    {
      rec_del(xo->dir, sizeof(MF), xo->pos, NULL);
      /* 本分類群組已被刪除，系統將自動移除捷徑 */
      vmsg("\xA5\xBB\xA4\xC0\xC3\xFE\xB8\x73\xB2\xD5\xA4\x77\xB3\x51\xA7\x52\xB0\xA3\xA1\x41\xA8\x74\xB2\xCE\xB1\x4E\xA6\xDB\xB0\xCA\xB2\xBE\xB0\xA3\xB1\xB6\xAE\x7C");
      return mf_load(xo);
    }
    return mf_init(xo);
  }
  ...
  XO *xo;
{
  ...
  /* 拷貝完成，但是卷宗內的資料不會被拷貝 */
  zmsg("\xAB\xFE\xA8\xA9\xA7\xB9\xA6\xA8\xA1\x41\xA6\xFD\xAC\x4F\xA8\xF7\xA9\x76\xA4\xBA\xAA\xBA\xB8\xEA\xAE\xC6\xA4\xA3\xB7\x7C\xB3\x51\xAB\xFE\xA8\xA9");
  ...
  XO *xo;
{
  ...
  if (mf.mftype & MF_FOLDER)
  {
    ...
    {
      /* 資料有誤，請重新複製後再貼上 */
      vmsg("\xB8\xEA\xAE\xC6\xA6\xB3\xBB\x7E\xA1\x41\xBD\xD0\xAD\xAB\xB7\x73\xBD\xC6\xBB\x73\xAB\xE1\xA6\x41\xB6\x4B\xA4\x57");
      return XO_FOOT;
    }
  }
